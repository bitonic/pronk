...
writeReport template h elapsed a@Analysis{..} = do
  let context "include"     = return $
                              MuLambdaM $ R.includeFile [templateDir, R.templateDir]
      ...
      context "latKde"      = return $ R.vector2 "time" "pdf" latKdeTimes latKdePDF
      context "latValues"   = return $ MuList . map mkGenericContext . G.toList $ lats
